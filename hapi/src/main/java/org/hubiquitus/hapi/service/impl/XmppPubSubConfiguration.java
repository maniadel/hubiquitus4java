 ...
 *
 */
public class XmppPubSubConfiguration {

	private Logger logger = LoggerFactory.getLogger(XmppPubSubConfiguration.class);
    ...
    * Constructor 
    */
    public XmppPubSubConfiguration() {
    	super();
    }
    ...
    public boolean isXmppDebugEnable() {
    	return Boolean.TRUE.equals((String) xmppConfigMap.get("xmpp.debug.enable"));
    }
    ...
    public String getXmppConfigurationXML() {
        String ret = null;
		try {
			ret = ObjectMapperInstanceDomainKey.BOT_CONFIGURATION.getMapper().writeValueAsString(this.xmppConfigMap);
		} catch (Exception e) {
			logger.warn("Could not convert XmppConfiguration to json", e);
		} 
        return ret;
    }
